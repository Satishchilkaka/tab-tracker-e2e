...
export class AddNewSongs {
    ...
    updateTitle() {
        cy.get(GlobalSelectors.inputFields).type('TEST')
        cy.get(GlobalSelectors.button).eq(7).click()
        cy.intercept('POST', '**/songs/*', { fixture: 'song.json' })
        ...
        cy.intercept('POST', '**/songs/*', { fixture: 'update.json' })
    }
}